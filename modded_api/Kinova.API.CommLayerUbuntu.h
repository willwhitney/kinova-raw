...
extern "C" int InitCommunication(void);

// extern "C" int InitCommunicationEthernet(EthernetCommConfig & config);

extern "C" int CloseCommunication(void);



extern "C" Packet SendPacket(Packet &packetOut, Packet &packetIn, int &result);

// extern "C" int SendPacketList(std::vector<Packet> & listPacket, int &result);

// extern "C" Packet BroadcastSendPacket(Packet &dataOut, std::vector<Packet> &dataIn, int &result, std::vector<unsigned long> &ipAddresses, int &count);

// extern "C" int InitDataStructures();

extern "C" int ScanForNewDevice();

extern "C" int GetDevices(KinovaDevice list[MAX_KINOVA_DEVICE], int &result);

extern "C" int SetActiveDevice(KinovaDevice device);

extern "C" int GetActiveDevice(KinovaDevice &device);
...
*/
extern "C" int OpenRS485_Read(RS485_Message PackagesIn [50], int QuantityWanted, int &ReceivedQtyIn);

extern "C" int OpenRS485_Write(RS485_Message PackagesOut [50] , int QtyToSend, int &QtySent);

extern "C" int OpenRS485_Activate(void);
